...
export function registerEngineTools(server: McpServer) {
  ...
  server.registerTool("validate_engines", { title: "Validate engines", description: "Validate local engine configuration and executable availability.", inputSchema: configInput.strict() }, (input) => mcpToolResult(() => { const parsed = configInput.parse(input); const runtime = createNodeEngineCatalogRuntime(); return jsonResult(validateEngineCatalog(loadEngineCatalog(parsed.config ?? resolveEngineCatalogPath(), runtime), runtime)); }));
}
