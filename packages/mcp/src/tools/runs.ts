...
export function registerRunTools(server: McpServer, options: Omit<OpenMcpContextOptions, "requireActor">) {
  ...
  server.registerTool("list_runs", {
      _meta: toolGroups("orchestration"), title: "List runs", description: "List autonomous coding runs.", inputSchema: listRunsInputSchema.strict() }, (input) => mcpToolResult(() => withMcpContext({ ...options, requireActor: false }, ({ context }) => jsonResult(listRuns(context, listRunsInputSchema.parse(input))))));
  server.registerTool("get_run", {
      _meta: toolGroups("orchestration"), title: "Get run", description: "Read an autonomous coding run.", inputSchema: runRefSchema.strict() }, (input) => mcpToolResult(() => withMcpContext({ ...options, requireActor: false }, ({ context }) => jsonResult(getRun(context, runRefSchema.parse(input).run)))));
  ...
  server.registerTool("stop_run", {
      _meta: toolGroups("orchestration"), title: "Stop run", description: "Request graceful or forced stop of an active run.", inputSchema: stopSchema.strict() }, (input) => mcpToolResult(() => withMcpContext({ ...options, requireActor: true }, ({ context }) => { const parsed = stopSchema.parse(input); return jsonResult(requestRunStop(context, parsed.run, parsed.force)); })));
  server.registerTool("retry_run", {
      _meta: toolGroups("orchestration"), title: "Retry run", description: "Create a new attempt for blocked or stalled work.", inputSchema: retryRunInputSchema.strict() }, (input) => mcpToolResult(() => withMcpContext({ ...options, requireActor: true }, ({ context }) => jsonResult(retryRun(context, retryRunInputSchema.parse(input))))));
  server.registerTool("resume_run", {
      _meta: toolGroups("orchestration"), title: "Resume run", description: "Resume an exact provider session when the adapter supports it.", inputSchema: runRefSchema.strict() }, (input) => mcpToolResult(() => withMcpContext({ ...options, requireActor: true }, ({ context }) => jsonResult(resumeRun(context, runRefSchema.parse(input).run)))));
  ...
  server.registerTool("list_run_artifacts", {
      _meta: toolGroups("orchestration"), title: "List run artifacts", description: "List structured artifact metadata.", inputSchema: runRefSchema.strict() }, (input) => mcpToolResult(() => withMcpContext({ ...options, requireActor: false }, ({ context }) => jsonResult(getRun(context, runRefSchema.parse(input).run).artifacts))));
  server.registerTool("archive_run", {
      _meta: toolGroups("orchestration"), title: "Archive run", description: "Archive terminal structured run history.", inputSchema: runRefSchema.strict() }, (input) => mcpToolResult(() => withMcpContext({ ...options, requireActor: true }, ({ context }) => jsonResult(archiveRun(context, runRefSchema.parse(input).run)))));
