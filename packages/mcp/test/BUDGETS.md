...
| requirements | `get_work_context` must stay within its budget, return the complete acceptance criteria, and report every truncated section in `omissions`. Then the complete-content path rebuilds the body and all 30 comments byte for byte through `read_issue_section`, and returns every hub relation. |
